...
public abstract class CameraBridgeViewBase extends SurfaceView implements SurfaceHolder.Callback {
    ...
    protected class CvCameraViewListenerAdapter implements CvCameraViewListener2 {
        ...
        public Mat onCameraFrame(Mat inputMat) {
            ...
            switch (mPreviewFormat) {
                case RGBA:
                    result = mOldStyleListener.onCameraFrame(inputMat);
                    break;
                case GRAY:
    ...
    public static void centerCrop(Mat mat, int newWidth, int newHeight) {
        resize(mat, newWidth, newHeight);

        int startX = (mat.width() - newWidth) / 2;
        int startY = (mat.height() - newHeight) / 2;

        org.opencv.core.Rect cropRect = new org.opencv.core.Rect(startX, startY, newWidth, newHeight);
        Mat temp = mat.submat(cropRect);
        temp.copyTo(mat);
        temp.release();
    }

    public static void resize(Mat mat, int maxWidth, int maxHeight) {
        double scale = Math.max((double) maxWidth / mat.width(), (double) maxHeight / mat.height());
        Imgproc.resize(mat, mat, new Size(0, 0), scale, scale);
    }
    ...
    protected void deliverAndDrawFrame(CvCameraViewFrame frame) {
        ...
        centerCrop(modified, getWidth(), getHeight());
